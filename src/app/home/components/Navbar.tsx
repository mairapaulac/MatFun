...
export default function Navbar() {
  ...
  const displayName = user?.name
    ...
    : "Carregando...";
  const classInfo = user?.gradeName && user?.classLetter
    ? `${user.gradeName}, Turma ${user.classLetter}`
    : " ";
        ...
        <div className="flex-grow flex justify-center items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="text-white text-center">
              <h2 className="text-2xl sm:text-4xl font-normal drop-shadow-md">
                {displayName}
              </h2>
              <p className="text-lg sm:text-lg text-white/80">{classInfo}</p>
            </div>
          </div>
        </div>
